...
export default function VerificarDocumento() {
      ...
      if (ordenEncontrada) {
        ...
        if (resultadosData && resultadosData.length > 0) {
          ...
          const mapeados = resVal.map(r => ({
             ...
             unidad: r.unidad || "",
             raw: r // Guardamos los datos crudos para poder ordenar
          }));
          listaValidados = [...listaValidados, ...mapeados];
        }

        // 5. Extraer Resultados de Cultivos
        ...
        // --- ALGORITMO DE ORDENAMIENTO (Idéntico al PDF) ---
        listaValidados.sort((a, b) => {
          // 1ro: Ordenar por Área (si existe la columna)
          const ordenAreaA = a.raw.orden_area || 0;
          const ordenAreaB = b.raw.orden_area || 0;
          if (ordenAreaA !== ordenAreaB) return ordenAreaA - ordenAreaB;

          // 2do: Ordenar por Examen / Perfil
          const ordenExamenA = a.raw.orden_examen || 0;
          const ordenExamenB = b.raw.orden_examen || 0;
          if (ordenExamenA !== ordenExamenB) return ordenExamenA - ordenExamenB;

          // 3ro: Ordenar por el orden visual del analito específico
          const visualA = a.raw.orden_visual || a.raw.orden || a.raw.id || 0;
          const visualB = b.raw.orden_visual || b.raw.orden || b.raw.id || 0;
  ...
  // CONTENEDOR PRINCIPAL: FORZAMOS MODO CLARO (Fondo Blanco, Texto Oscuro)
      ...
      <div style={{ maxWidth: "650px", margin: "0 auto" }}>
        ...
        <div style={{ marginTop: "30px", border: "1px solid #cbd5e1", borderRadius: "8px", overflow: "hidden", boxShadow: "0 4px 6px -1px rgba(0, 0, 0, 0.05)" }}>
          <div style={{ backgroundColor: "#1F355E", padding: "15px", color: "white", fontWeight: "bold", fontSize: "15px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: "11px", backgroundColor: "#0284c7", padding: "4px 10px", borderRadius: "12px", border: "1px solid #38bdf8", letterSpacing: "0.5px", color: "#ffffff" }}>INALTERABLES</span>
          </div>
          
          {resultados.length > 0 ? (
            /* Ajuste del scroll: 60vh asegura que se adapte al alto de pantallas móviles o monitores */
            <div style={{ backgroundColor: "white", maxHeight: "60vh", minHeight: "150px", overflowY: "auto" }}>
